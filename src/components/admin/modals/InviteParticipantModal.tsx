...
export function InviteParticipantModal({ isOpen, onClose, onSuccess }: InviteParticipantModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch("/api/admin/participants/invite", {
        method: "POST",
        headers: {
      ...
      if (!res.ok) {
        throw new Error((json && json.error) || "Failed to invite participant");
      }
      ...
      setLocalError(err instanceof Error ? err.message : "Failed to invite participant");
    } finally {
      setIsSaving(false);
    }
  };
