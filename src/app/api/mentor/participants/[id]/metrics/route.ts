...
export async function GET(
  ...
  if (pseudonymId) {
    ...
    const { data: metrics, error: metricsError } = await admin
      .from("garmin_metrics")
      .select(
        "id, metric_date, resting_heart_rate, average_stress_level, sleep_duration_seconds, sleep_score, body_battery_charged, body_battery_drained, body_battery_most_recent, body_battery_start, body_battery_lowest, hrv_last_night_average, hrv_last_night_5_min_high"
      )
      .eq("pseudonym_id", pseudonymId)
