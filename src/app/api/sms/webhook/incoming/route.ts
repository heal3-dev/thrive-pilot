...
export async function POST(request: Request) {
  ...
  // Step 8: Best-effort enqueue assigned mentor email alert (rate-limited via idempotency key).
  // Avoid blocking Twilio on delivery; any failure here should not cause retries.
  if (messageType === "user_message") {
    try {
      ...
      if (!mentorErr && mentorRow?.email && mentorRow.is_active !== false) {
        ...
        const idempotencyKey = `mentor_inbound_alert:${assignment.mentor_id}:${participant.id}:${bucket}`;

        // Insert a pending email job. Idempotency key prevents duplicates within the cooldown window.
        await supabase.from("email_jobs").upsert(
          {
            kind: "mentor_inbound_alert",
            to_email: mentorRow.email,
      ...
      console.warn("Inbound SMS: enqueue mentor alert email failed (ignored)", {
        messageSid,
