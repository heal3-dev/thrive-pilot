...
function randomToken(): string {
  // 32 bytes -> 43 char base64url string (short enough for SMS).
...
export async function POST(request: Request) {
  ...
  if ("action" in payload && payload.action === "revoke") {
    let q = admin
      ...
      .eq("weekly_report_id", payload.reportId)
      .eq("is_active", true);
  ...
  // Ensure report exists (helps return a clean 404).
  ...
  // Reuse existing active link if still valid.
