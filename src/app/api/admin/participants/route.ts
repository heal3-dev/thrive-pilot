...
export async function GET(request: Request) {
  ...
  const { data: participantsData, error: participantsError } = await admin
    .from("participants")
    .select(
      "id, name, phone_number, email, is_active, garmin_user_id, consent_given, created_at, updated_at, weekly_report_sms_enabled, weekly_report_email_enabled"
    )
  ...
  if (metricsData) {
    for (const m of metricsData) {
      ...
      metricsByParticipant.get(participantId)?.push({
        ...
        body_battery_start: (m as unknown as { body_battery_highest?: number | null; body_battery_start?: number | null }).body_battery_highest
          ?? (m as unknown as { body_battery_start?: number | null }).body_battery_start
          ?? null,
