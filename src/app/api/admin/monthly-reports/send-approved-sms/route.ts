...
export async function POST(request: Request) {
  ...
  let query = guard.admin
    ...
    .select(
      "id, participant_id, month_ending, month_range, badge_label, badge_icon, outreach_text, status, approved_at, html, sms_message_id",
    )
  ...
  for (const report of rows) {
    ...
    const baseRaw =
      typeof report.outreach_text === "string" && report.outreach_text.trim().length > 0
        ...
        : `Hi ${firstName(participantName)}, this month your Thrive monthly report (${report.month_range}) shows that you flagged ${report.badge_label} ${report.badge_icon}. Let us know if you have any questions after you’ve reviewed the report.`;

    const base = baseRaw
      .replace(/\b(Thrive\s+monthly\s+report)\s*\(([^)]*)\)/i, `$1 (${report.month_range})`)
      .replace(/\n+\s*Monthly report:\s*\S+\s*$/i, "")
      .trim();

    const msg = `${base}\n\nMonthly report: ${shareUrl}`;
