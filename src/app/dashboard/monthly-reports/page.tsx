...
import { DEFAULT_MONTHLY_GENERATE_WRAPPER, DEFAULT_MONTHLY_MASTER_RULES, DEFAULT_MONTHLY_OLGA_HTML_BASE_TEMPLATE, DEFAULT_MONTHLY_REVISE_WRAPPER } from "@/lib/weekly-reports/template-defaults";
...
type TemplateKey = "monthly_master_rules" | "monthly_revise_wrapper" | "monthly_generate_wrapper" | "monthly_html_base_template";
type TemplateRow = {
  id: string;
  key: TemplateKey;
  content: string;
...
const TEMPLATE_LABELS: Record<TemplateKey, { title: string; hint: string }> = {
  monthly_master_rules: {
    title: "Master Prompt",
    hint: "Comprehensive Thrive Monthly Report instructions here (tone, structure, badge labels, fixed closing line).",
  },
  monthly_revise_wrapper: {
    title: "Revise Prompt",
...
const DEFAULT_TEMPLATES: Record<TemplateKey, string> = {
  monthly_master_rules: DEFAULT_MONTHLY_MASTER_RULES,
  monthly_revise_wrapper: DEFAULT_MONTHLY_REVISE_WRAPPER,
  monthly_generate_wrapper: DEFAULT_MONTHLY_GENERATE_WRAPPER,
  monthly_html_base_template: DEFAULT_MONTHLY_OLGA_HTML_BASE_TEMPLATE,
};
...
function getDisplayStatus(
  ...
  report: MonthlyReportMini | null | undefined
): DisplayStatus {
  if (status === "draft") {
...
export default function MonthlyReportsPage() {
  ...
  const [metaByParticipant, setMetaByParticipant] = useState<Record<string, MonthlyReportMeta>>({});
  ...
  const [reportsByParticipant, setReportsByParticipant] = useState<Record<string, MonthlyReportMini>>({});
  ...
  const [sendPreview, setSendPreview] = useState<null | {
    ...
    alreadyQueued: Array<{ reportId: string; participantId: string; monthRange: string }>;
  }>(null);
  ...
  const [smsPreview, setSmsPreview] = useState<null | {
    toSend: Array<{ reportId: string; participantId: string; participantName: string; toPhone: string; monthRange: string; shareUrl: string; expiresAt: string }>;
  }>(null);
  const [selectedSmsReportIds, setSelectedSmsReportIds] = useState<Record<string, boolean>>({});

  const [isTemplatesOpen, setIsTemplatesOpen] = useState(false);
  const [templateKey, setTemplateKey] = useState<TemplateKey>("monthly_master_rules");
  ...
  const LAYOUT_STORAGE_KEY = "monthlyReportsLayout.v1";
  ...
  const formatReportTitleLine = useCallback((meta: MonthlyReportMeta) => {
    return `Thrive Monthly Report ${meta.participantLabel} ${meta.monthRange} ${meta.badgeLabel} ${meta.badgeIcon}.`;
  }, []);

  const buildOutreachText = useCallback((meta: MonthlyReportMeta) => {
    const first = meta.participantLabel.trim().split(/\s+/)[0] || meta.participantLabel.trim();
    return [
      `Hi ${first}, this month your Thrive monthly report (${meta.monthRange}) shows that you flagged ${meta.badgeLabel} ${meta.badgeIcon}.`,
  ...
  useEffect(() => {
    ...
    async function ensureShareUrl() {
      ...
      try {
        ...
        const res = await fetch("/api/admin/monthly-reports/share", {
          method: "POST",
          headers: {
  ...
  const loadTemplates = useCallback(
    async (force: boolean) => {
      ...
      try {
        ...
        const res = await fetch(`/api/admin/monthly-reports/templates?keys=${encodeURIComponent(TEMPLATE_KEYS.join(","))}`, {
  ...
  const refreshParticipantStatuses = useCallback(async () => {
    try {
      ...
      const chunkSize = 150;
      const merged: Record<string, MonthlyReportMini> = {};
      for (let i = 0; i < ids.length; i += chunkSize) {
        const chunk = ids.slice(i, i + chunkSize);
        const res = await fetch("/api/admin/monthly-reports/list", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
  ...
  useEffect(() => {
    ...
    const handle = window.setTimeout(async () => {
      try {
        ...
        const res = await fetch("/api/admin/monthly-reports/report", {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
          ...
          body: JSON.stringify({
            participantId: pid,
            monthEnding: selectedMeta.monthEnding,
  ...
  useEffect(() => {
    ...
    async function ensureGenerated() {
      ...
      try {
        ...
        const existingRes = await fetch(`/api/admin/monthly-reports/report?participantId=${encodeURIComponent(p.id)}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        if (existingRes.ok) {
          ...
          if (report && typeof report === "object") {
            ...
            if (html.trim().length > 0) {
              ...
              const monthEnding = typeof r.month_ending === "string" ? r.month_ending : "";
        ...
        const statusUrl = `/api/admin/monthly-reports/data-status?participantId=${encodeURIComponent(p.id)}`;
        ...
        const res = await fetch("/api/admin/monthly-reports/generate", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
        ...
        if (!res.ok) {
          ...
          const msg =
            ...
            typeof (j as { error?: unknown }).error === "string"
              ? (j as { error: string }).error
              : null;
          throw new Error(msg || "Failed to generate monthly report");
        }
        ...
        const nextMeta =
          j.reportId && j.monthRange && j.badgeLabel && j.badgeIcon
            ? ({
                reportId: j.reportId,
                participantLabel: j.participantLabel || formatParticipantLabel(p),
                monthEnding: j.monthEnding || "",
                monthRange: j.monthRange,
                badgeLabel: j.badgeLabel,
                badgeIcon: j.badgeIcon,
              } as MonthlyReportMeta)
            : null;

        const assistantMsg: ChatMessage = {
          id: createId("gen"),
          role: "assistant",
          content: nextMeta ? formatReportTitleLine(nextMeta) : "Generated the monthly report draft.",
        };
    ...
    selectedParticipant,
    templateDraftByKey.monthly_html_base_template,
    templatesByKey.monthly_html_base_template?.content,
    htmlByParticipant,
  ...
  return (
    <div className="h-full min-h-0 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5 flex flex-col gap-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Button
            variant="outline"
            size="sm"
            onClick={() => router.push("/dashboard/monthly-reports/demo")}
          ...
          <Button
            ...
            onClick={async () => {
              ...
              try {
                ...
                const token = sessionData?.session?.access_token;
                if (!token) throw new Error("Authentication required");
                const res = await fetch("/api/admin/monthly-reports/send-approved", {
                  method: "POST",
                  headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${token}`,
                ...
                const preview = {
                  toEnqueue: toEnqueue as Array<{ reportId: string; participantId: string; toEmail: string; participantName: string; monthRange: string }>,
                  alreadyQueued: alreadyQueued as Array<{ reportId: string; participantId: string; monthRange: string }>,
                };
          ...
          <Button
            ...
            onClick={async () => {
              ...
              try {
                ...
                const token = sessionData?.session?.access_token;
                if (!token) throw new Error("Authentication required");
                const res = await fetch("/api/admin/monthly-reports/send-approved-sms", {
                  method: "POST",
                  headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${token}`,
                    ...
                    <div key={it.reportId} className="flex items-center justify-between gap-3 px-4 py-3 border-b border-slate-100">
                      <label className="flex items-start gap-3 flex-1 min-w-0">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="text-xs text-slate-500 mt-0.5">{it.monthRange}</p>
                        </div>
                      </label>
                      <Button
                        type="button"
                        ...
                        onClick={async (e) => {
                          ...
                          try {
                            ...
                            const token = sessionData?.session?.access_token;
                            if (!token) throw new Error("Authentication required");
                            const res = await fetch("/api/admin/monthly-reports/report/skip", {
                              method: "POST",
                              headers: {
                                "Content-Type": "application/json",
                                Authorization: `Bearer ${token}`,
      ...
      <Modal
        ...
        title="Monthly Report Templates"
      ...
      >
        <div className="flex flex-col gap-4 h-full min-h-0">
          ...
          <div className="sticky bottom-0 -mx-4 sm:-mx-6 px-4 sm:px-6 py-3 bg-white border-t border-slate-100 flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              {!currentTemplateRow && (templateKey === "monthly_html_base_template" || templateKey === "monthly_master_rules") ? (
                <Button
                  variant="outline"
                  onClick={async () => {
                    setTemplatesError(null);
                    ...
                    try {
                      ...
                      const res = await fetch("/api/admin/monthly-reports/templates/seed", {
                        method: "POST",
                        headers: {
                          "Content-Type": "application/json",
                          Authorization: `Bearer ${token}`,
              ...
              <Button
                onClick={async () => {
                  ...
                  try {
                    ...
                    const res = await fetch("/api/admin/monthly-reports/templates", {
                      method: "PUT",
                      headers: {
                        "Content-Type": "application/json",
                        Authorization: `Bearer ${token}`,
                ...
                <Button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      const res = await fetch("/api/admin/monthly-reports/report/approve", {
                        method: "POST",
                        headers: {
                          "Content-Type": "application/json",
                          Authorization: `Bearer ${token}`,
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        const statusRes = await fetch(
                          `/api/admin/monthly-reports/data-status?participantId=${encodeURIComponent(selectedParticipant.id)}`,
                          { headers: { Authorization: `Bearer ${token}` } }
                        );
                        if (statusRes.ok) {
                    ...
                    <Button
                      onClick={async () => {
                        ...
                        try {
                          ...
                          const participantLabel = formatParticipantLabel(selectedParticipant);
                          const res = await fetch("/api/admin/monthly-reports/chat", {
                            method: "POST",
                            headers: {
                              "Content-Type": "application/json",
                              Authorization: `Bearer ${token}`,
